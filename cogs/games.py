...
class Blackjack(discord.ui.View):
    def __init__(self, bot, ctx: commands.Context, owner: Player.player, bet_amount):
        ...
        self.file = discord.File(
            "./cogs/assets/cards-clipart.png", filename="cards.png"
        )
        self.player = owner
    ...
    async def create_embed(self) -> discord.Embed:
        ...
        embed.add_field(
            name="Player cards",
            value=f"{p_cards} ({Deck.get_values(self.player_hand)})",
        )
        embed.add_field(
            name="Dealer cards", value=f"{str(self.dealer_hand[0])}, ?", inline=False
        )
    ...
    async def update_embed(
        ...
        embed = (
            self.embed
            if self.embed != interaction.message.embeds[0]
            else interaction.message.embeds[0]
        )
        p_cards = ", ".join([f"{str(c)}" for c in self.player_hand])
        embed.set_field_at(
            0,
        ...
        if show_dealer_cards:
            ...
            embed.set_field_at(
                1,
                name="Dealer cards",
                value=f"{d_cards} ({Deck.get_values(self.dealer_hand)})",
                inline=False,
            )
        else:
            embed.set_field_at(
                1,
                name="Dealer cards",
                value=f"{str(self.dealer_hand[0])}, ?",
                inline=False,
            )
    ...
    async def win(
        ...
        await self.update_embed(
            interaction, message_text=f"{str(self.player)} won!", show_dealer_cards=True
        )
        self.stop()
    ...
    async def lose(self, interaction: discord.Interaction, button_clicked: int):
        ...
        await self.bot.stats.add(
            "totalbjLost", (abs(self.player_bet))
        )  # adds to totalbjLost stat
        await self.update_embed(
            interaction,
            message_text=f"{str(self.player)} lost.",
    ...
    async def push(self, interaction: discord.Interaction, button_clicked: int):
        ...
        await self.update_embed(
            interaction,
            message_text=f"{str(self.player)} pushed!",
            show_dealer_cards=True,
        )
        self.stop()
    ...
    async def blackjack(self, message: discord.Message):
        ...
        await message.edit(
            content=f"BLACKJACK! {str(self.player)}",
            attachments=[],
            embed=self.embed,
            view=self,
        )
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.player.id:
            return await interaction.response.send_message(
                "This isn't your game of Blackjack.", ephemeral=True
            )
        self.stop()
...
class games(commands.Cog, command_attrs=dict(name="Games")):
    ...
    @commands.command(
        aliases=["b"],
        description="Bets money. There is a 50/50 chance on winning and losing.",
    )
    async def bet(self, ctx, amount):
        ...
        if amount < 0:
            await self.bot.stats.add(
                "totalbetLost", (abs(amount))
            )  # adds to totalbetLost stat if amount is negative (player lost money)
        await ctx.send(
            f"You {['lost', 'won'][y]} ${abs(amount)}, new balance: {player.balance + amount}"
        )
    ...
    async def roulette(self, ctx, amount):
        ...
        if result[0] == rlist[2]:  # green
            ...
            return await ctx.send(
                f"Your choice: {str(reaction.emoji)}, result: {str(result[0])}\n**You won ${int(amount * 35)}!** <a:cookdance:829764800758022175>"
            )
            ...
            await self.bot.stats.add(
                "totalrouletteLost", (abs(amount))
            )  # adds to totalrouletteLost stat
            return await ctx.send(
                f"Your choice: {str(reaction.emoji)}, result: {str(result[0])}\nYou lost ${int(amount)}."
            )

    @commands.group(
        aliases=["rps"], invoke_without_command=True, description="Rock paper scissors."
    )
    async def rockpaperscissors(self, ctx, amount, choice):
        ...
        if not choice.strip().lower() in choices:
            return await ctx.send(
                "It looks like you didn't specify `rock`, `paper`, or `scissor` (singular, no 's)."
            )
        ...
        if outcome_list[rand] == "tied with me":
            ...
            await player.update_balance((0 - amount), ctx)
            await self.bot.stats.add(
                "totalrpsLost", (abs(amount))
            )  # adds to totalrpsLost stat
        ...
        while not guessed_it:
            ...
            try:
                ...
                if content == the_number:
                    await ctx.send(
                        f"You guessed it! The number was {the_number}.\nAs promised, here's your winnings of ${amount}."
                    )
                    await player.update_balance(amount, ctx)
                    return
                else:
                    ...
                    if tries == 0:
                        await ctx.send(
                            f"You're out of tries, and haven't guessed my number.\nYou lose ${amount}."
                        )
                        await player.update_balance((0 - amount), ctx)
                        await self.bot.stats.add(
                            "totalguessLost", (abs(amount))
                        )  # adds to totalguessLost stat
                        return
                ...
                if "cancel" in content:
                    await ctx.send(
                        "Guessing game cancelled. You did not lose or gain any money."
                    )
                    return
                else:
    ...
    async def blackjack(self, ctx, amount):
        ...
        if await bjview.wait():
            ...
            await ctx.send(
                f"{ctx.author.mention}, your blackjack game timed out.", delete_after=45
            )
    ...
    async def craps(self, ctx, amount):
        ...
        async def win(amount: int, dice_1, dice_2, main_message) -> None:
            ...
            try:
                await main_message.edit(embed=updated_embed, view=X())
            except (ReferenceError, AttributeError):
        ...
        async def loss(amount: int, dice_1, dice_2, main_message) -> None:
            ...
            await self.bot.stats.add(
                "totalcrapsLost", (abs(amount))
            )  # adds to totalcrapsLost stat
            try:
                updated_embed = main_message.embeds[0]
            except (ReferenceError, AttributeError):
                updated_embed = discord.Embed(title="Craps", description="").set_footer(
            ...
            while nds != 7:
                ...
                try:
                    await main_message.edit(
                        embed=updated_embed.set_footer(
                            text=f"Amount bet: {amount} | Rolls: {rolls}"
                        )
                    )
