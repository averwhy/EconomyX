...
class Blackjack(discord.ui.View):
    def __init__(self, bot, owner: discord.Member, bet_amount):
        super().__init__()
        self.bot = bot
        self.values = values = {'Two':2, 'Three':3, 'Four':4, 'Five':5, 'Six':6, 'Seven':7, 'Eight':8, 'Nine':9, 'Jack':10,'Queen':10, 'King':10, 'Ace':11}
        self.value = None
    ...
    async def update_embed(self, interaction: discord.Interaction):
        ...
        embed.set_field_at(1, name="Dealer cards", value=f"?, {self.dealer_cards[1]} ({self.dealer_hidden_total})", inline=False)
    ...
    async def win(self, interaction: Union[discord.Interaction, commands.Context], button_clicked: int, player_msg: str = "", dealer_msg: str = ""):
        ...
        embed.set_field_at(1, name="Dealer cards", value=f"{self.readable_dealer_cards()} ({self.dealer_total}) {dealer_msg}", inline=False)
        self.disable_all()
        self.children.pop(2)
    ...
    async def push(self, interaction: discord.Interaction, button_clicked: int, player_msg: str = "", dealer_msg: str = ""):
        ...
        embed.set_footer(text=f"No money lost or gained.")
        embed.set_field_at(0, name="Player cards", value=f"{self.player_cards[0]}, {self.player_cards[1]} ({self.player_total}) {player_msg}")
        embed.set_field_at(1, name="Dealer cards", value=f"{self.dealer_cards[0]}, {self.dealer_cards[1]} ({self.dealer_total}) {dealer_msg}", inline=False)
        self.disable_all()
        self.style_button(button_clicked, discord.ButtonStyle.success)
    ...
    async def stand(self, interaction: discord.Interaction, button: discord.ui.Button,):
        ...
        if self.dealer_total >= 17 and self.dealer_total < 21:
            ...
            #dealer instant bust
            await interaction.response.send_message(f"The dealer's hidden card was a {self.dealer_cards[0]}, which makes their card total {self.dealer_total}. Dealer bust!", ephemeral=True)
            ...
            if self.dealer_total > 21:
                ...
                newcard2 = random.choice(list(self.values.keys()))
                self.dealer_cards.append(newcard2)
                self.recalculate_totals()
                if self.dealer_total > 21:
                    #dealer busts
                    await interaction.response.send_message(f"The dealer drew 2 cards, {newcard} and {newcard2}. Their hidden card was a {self.dealer_cards[0]}, which makes their card total {self.dealer_total}. Dealer bust!", ephemeral=True)
                    await self.win(interaction, 1, dealer_msg="**Bust!**")
                    self.stop()
                elif self.dealer_total == 21:
                    #dealer blackjacks
