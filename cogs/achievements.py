...
AchievementsDict = {
    ...
    8: Achievement(
        "Stonks", "<:stonks:1071087769209806922> Buy your first share in stock"
    ),
    9: Achievement("Day trader", "Sell shares of a stock that total $1,000 in profit"),
    10: Achievement(
        "Wolf of Wall Street", "Sell shares of a stock that total $100,000 in profit"
    ),
    ...
    27: Achievement(
        "Successful History",
        "Reach a total earned amount of $1,000,000,000,000 (1 Trillion dollars)",
    ),
    28: Achievement(
        "EconomyX Enjoyer",
        "<:gigachad:1071094100998230047> Use 1000 commands with EconomyX",
    ),
    29: Achievement(
        "Dedicated User",
        "<:gigachad:1071094100998230047> Use 10,000 commands with EconomyX",
    ),
    30: Achievement(
        "#1 Fan", "Use 100,000 commands with EconomyX <:gigachad:1071094100998230047>"
    ),
}
...
class achievements(commands.Cog, command_attrs=dict(name="Achievements")):
    ...
    async def has_ach(
        self, player: typing.Union[discord.Member, discord.User, int], ach_id: int
    ):
        if type(player) != int:
            player = player.id
        c = await self.bot.pool.fetchrow(
            "SELECT * FROM e_achievements WHERE userid = $1 AND achid = $2",
            player,
            ach_id,
        )
        if c:
    ...
    async def give_ach(
        self, player: typing.Union[discord.Member, discord.User], ach_id: int
    ):
        if not await self.has_ach(player, ach_id):
            log.info(f"{player.name} already has achievement {ach_id}")
            return
        await self.bot.pool.execute(
            "INSERT INTO e_achievements(userid, achid) VALUES ($1, $2)",
            player.id,
            ach_id,
        )
        ...
        async with aiohttp.ClientSession() as session:
            ...
            await webhook.send(
                f"{player.name} got achievement **{(AchievementsDict[ach_id]).name}**! ({discord.utils.format_dt(now)})"
            )
    ...
    async def on_command_completion(self, ctx):
        ...
        if ctx.command.cog_name.lower() == "games":
            result = await self.bot.pool.fetchrow(
                "SELECT gamesPlayed from e_player_stats WHERE id = $1", ctx.author.id
            )
        ...
        if ctx.command.name == "work":
            result = await self.bot.pool.fetchrow(
                "SELECT level FROM e_jobs WHERE id = $1", ctx.author.id
            )
        ...
        if ctx.command.name == "invest":
            playerinvests = await self.bot.pool.fetch(
                "SELECT * FROM e_invests WHERE userid = $1", ctx.author.id
            )
        ...
        if ctx.command.name == "invest":
            playerinvests = await self.bot.pool.fetch(
                "SELECT stockid FROM e_invests WHERE userid = $1", ctx.author.id
            )
            playerstock = await self.bot.get_stock_from_player(ctx.author.id)
            if (
                playerstock is not None
            ):  # it can be none because some people will do e$invest without owning a stock
        ...
        if ctx.command.name == "pay":
            result = await self.bot.pool.fetchrow(
                "SELECT amountPaid from e_player_stats WHERE id = $1", ctx.author.id
            )
        ...
        if (
            len(earned_achievements) > 0
        ):  # if its not 0, an achievement criteria was met
            msg = ""
            for a in earned_achievements:
                await self.give_ach(ctx.author, a)
                msg = msg + (
                    f"**Achievement get!** {AchievementsDict[a].name}\n*See your achievements with `{ctx.clean_prefix}achievements`*\n"
                )
            await ctx.reply(msg)

    async def get_player_ach(self, player):
        result = await self.bot.pool.fetch(
            "SELECT * FROM e_achievements WHERE userid = $1", player.id
        )
        return tuple(result)

    @commands.group(
        invoke_without_command=True, name="achievements", aliases=["ach", "achieve"]
    )
