...
class statistics(commands.Cog, command_attrs=dict(name='Statistics')):
    ...
    async def add(self, stat_name: str, amount: int = 1):
        ...
        amount = int(amount)
        await self.bot.pool.execute("UPDATE e_bot_stats SET statvalue = (statvalue + $1) WHERE statname = $2", amount, stat_name,)
    ...
    async def user_add_games(self, player: typing.Union[discord.Member, discord.User], amount: int = 1):
        ...
        amount = int(amount)
        await self.bot.pool.execute(f"UPDATE e_player_stats SET gamesPlayed = (gamesPlayed + $1) WHERE id = $2", amount, player.id,)
    ...
    async def user_add_commands(self, player: typing.Union[discord.Member, discord.User], amount: int = 1):
        ...
        amount = int(amount)
        await self.bot.pool.execute(f"UPDATE e_player_stats SET commandsUsed = (commandsUsed + $1) WHERE id = $2", amount, player.id)
    ...
    async def user_add_paid(self, player: typing.Union[discord.Member, discord.User], amount: int = 1):
        ...
        amount = int(amount)
        await self.bot.pool.execute(f"UPDATE e_player_stats SET amountPaid = (amountPaid + $1) WHERE id = $2", amount, player.id)
    ...
    async def cog_load(self):
        ...
        if len(result) == 0:
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalLost')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalbetLost')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalbjLost')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalcrapsLost')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalguessLost')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalrpsLost')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalrouletteLost')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalinvestLost')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalstocksInvested')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalPaid')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalLotteries')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totallotteryTicketsBought')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalprofilesViewed')")
            await self.bot.pool.execute("INSERT INTO e_bot_stats(statname) VALUES ('totalCommands')")
