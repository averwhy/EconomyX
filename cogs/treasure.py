...
class Shovel:
    ...
    def dig(self, has_perk_2: bool = False) -> tuple:
        ...
        return random.choices(REWARDS, weights=[O.rarity for O in REWARDS], k=ore_count*quantity_multiplier)
...
PERKS = (
    ...
    Perk(3, 'Rock Polisher', 'increases revenue from sold ores by 30% one time', 1500, 1)
)
...
class TreasureShopButton(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.player.bal < self.cost:
            ...
            await interaction.response.send_message(f"You can't afford that upgrade! You need ${self.cost-self.player.bal} more.", ephemeral=True)
            return self.stop()
        self.value = True
        await interaction.message.edit(embed=interaction.message.embeds[0], view=self)
...
class PerkShopMenu(discord.ui.Select):
    def __init__(self):
        perklist = [discord.SelectOption(label=p.name, description=p.description, value=p.id) for p in PERKS]

        # The placeholder is what will be shown when no option is chosen
        # The min and max values indicate we can only pick one of the three options
        # The options parameter defines the dropdown options. We defined this above
        super().__init__(placeholder='Buy perks...', min_values=1, max_values=1, options=perklist)

    async def callback(self, interaction: discord.Interaction):
        conf = Confirm(disable_after_click=True)
        chosen_perk = Perk.get(int(self.values[0]))
        await interaction.response.send_message(f"Purchase perk '{chosen_perk.name}' for ${chosen_perk.cost}?", view=conf, ephemeral=True)
        await conf.wait()
...
class treasurehunting(commands.Cog, command_attrs=dict(name='Treasures')):
    ...
    def __init__(self, bot):
        ...
        self.none_found_messages = ("After a valiant effort, your search turns up treasure-less.",
                                    "Your luck ran dry..",
        ...
        self.found_messages = ("Your digging yielded success!",
    ...
    async def cog_check(self,ctx): #REMOVE ME WHEN DONE DEVELOPING THE FUMKING UPDATE!
        return ctx.author.id in [OWNER_ID, 295693363284410368]

    async def cog_load(self):
        await self.bot.pool.execute("""CREATE TABLE IF NOT EXISTS "e_treasure" 
        ...
        await self.bot.pool.execute("""CREATE TABLE IF NOT EXISTS "e_treasure_inventory" 
                                    (id bigint REFERENCES e_users(id),
                                    oreid smallint,
                                    minedwith smallint,
                                    dugwhen timestamp with time zone
                                    )""")
        await self.bot.pool.execute("""CREATE TABLE IF NOT EXISTS "e_treasure_shovel_stats" 
                                    (id bigint REFERENCES e_users(id),
                                    shovelid smallint,
                                    timesdug integer,
                                    moneyearned integer)""")
        await self.bot.pool.execute("""CREATE TABLE IF NOT EXISTS "e_treasure_perks" 
                                    (id bigint REFERENCES e_users(id),
                                    perkid smallint,
                                    usesleft smallint,
    ...
    def can_dig(self, data: tuple):
        ...
        log.debug(last_mins)
        cooldown = last_dug + timedelta(minutes=last_mins) # cooldown is 2-8 mins in the future from last worked
    ...
    async def treasure(self, ctx):
        ...
        total_diggers = tuple(await self.bot.pool.fetchrow("SELECT COUNT(*) FROM e_treasure"))[0]
        total_digs = tuple(await self.bot.pool.fetchrow("SELECT SUM(timesdug) FROM e_treasure"))[0]
        total_earnt = tuple(await self.bot.pool.fetchrow("SELECT SUM(moneyearned) FROM e_treasure_shovel_stats"))[0]

        player_desc = "" # get totalmoney, shovel, and times dug
        player_data = await self.bot.pool.fetchrow("SELECT * FROM e_treasure WHERE ID = $1", ctx.author.id)
        if player_data is None:
            player_desc = f"You haven't dug for treasure before! Try {ctx.clean_prefix}dig"
        else:
    ...
    async def shop(self, ctx):
        ...
        await ctx.send(f"Usage: `{ctx.clean_prefix}shop (tools/perks)` <:blobthumbsup:819979852576587907>")
    
    @shop.command()
    async def tools(self, ctx):
        player = await Player.get(ctx.author.id, self.bot)
        ownedShovels = await self.bot.pool.fetch("SELECT shovelid FROM e_treasure_shovel_stats WHERE id = $1", ctx.author.id)
        ownedIDs = [s.get('shovelid') for s in ownedShovels]
        upperlimit = max(ownedIDs) + 7
        toollist = list([(f"[${t.price}] {t.name} ({t.multiplier}) {CHECK if t.id in ownedIDs else ''}") for t in SHOVELS if t.id < upperlimit])
        alltools = "\n".join(toollist)
        embed = discord.Embed(title="⛏ Treasure Tool Shop", description=f"-# [price] (name) (quality)\n```ml\n{alltools}\nand {len(SHOVELS)-upperlimit} more...\n```",color=player.profile_color)
        ...
        if shopview.value:
            ...
            await self.bot.pool.execute("UPDATE e_treasure SET currentshovel = $1 WHERE id = $2", next_tool.id, player.id)
            await self.bot.pool.execute("INSERT INTO e_treasure_shovel_stats(id, shovelid, timesdug, moneyearned) VALUES ($1, $2, 0, 0)", player.id, next_tool.id)
            await shopmsg.add_reaction('\U00002705')
    
    @shop.command()
    async def perks(self, ctx):
        player = await Player.get(ctx.author.id, self.bot)
        activePerks = await self.bot.pool.fetch("SELECT perkid FROM e_treasure_perks WHERE id = $1", ctx.author.id)
        activeIDs = [s.get('perkid') for s in activePerks]
        perklist = list([(f"[{p.id}] {p.name} ({p.description}) {CHECK if p.id in activeIDs else ''}") for p in PERKS])
        allperks = "\n".join(perklist)
        embed = discord.Embed(title="⛏ Treasure Perk Shop", description=f"-# [id] (name) (description)\n```ml\n{allperks}```",color=player.profile_color)
        ...
        if len(perkview.children[0].values) >= 1:
            ...
            if chosen_perk.id in activeIDs:
                return await ctx.reply(f"You already have '{chosen_perk.name}'! Use it before buying another.")
            await player.update_balance(amount=(chosen_perk.cost*-1), ctx=ctx)
            await self.bot.pool.execute("UPDATE e_treasure SET currentshovel = $1 WHERE id = $2", chosen_perk.id, player.id)
            await self.bot.pool.execute("INSERT INTO e_treasure_perks(id, perkid, usesleft) VALUES ($1, $2, $3)", player.id, chosen_perk.id, chosen_perk.uses)
            await shopmsg.add_reaction('\U00002705')
    ...
    async def dig(self, ctx):
        ...
        player_treasure_data = await self.bot.pool.fetchrow("SELECT * FROM e_treasure WHERE id = $1", player.id,)
        if not player_treasure_data:
            #no player!
            await self.bot.pool.execute("INSERT INTO e_treasure(id, currentshovel, timesdug, lastmins) VALUES ($1, 0, 0, 0)", player.id)
            await self.bot.pool.execute("INSERT INTO e_treasure_shovel_stats(id, shovelid, timesdug, moneyearned) VALUES ($1, 0, 0, 0)", player.id)
            player_treasure_data = await self.bot.pool.fetchrow("SELECT * FROM e_treasure WHERE id = $1", player.id,)
            await ctx.send(f"You've dug for treasure for the first time! View your stats with `{ctx.clean_prefix}treasure`")
        elif not self.can_dig(player_treasure_data):
            return await ctx.send(f"You're too tired to dig right now.. <:blobfeelsbad:819979895451025428>.. {discord.utils.format_dt(player_treasure_data[3] + timedelta(minutes=player_treasure_data[4]), 'R')}")

        player_shovel = Shovel.get(player_treasure_data[1])
        player_perks = await self.bot.pool.fetch("SELECT * FROM e_treasure_perks WHERE id = $1", ctx.author.id)
        ...
        if 2 in perk_ids:
            ...
            if perk_2.get('usesleft') == 0:
                await self.bot.pool.execute("DELETE FROM e_treasure_perks WHERE id = $1 AND perkid = 2", ctx.author.id)
            else:
                has_perk_2 = True
                perk_message = f" - *Metal Detector active, {perk_2.get('usesleft')} uses left*"
                await self.bot.pool.execute("UPDATE e_treasure_perks SET usesleft = (usesleft) - 1 WHERE id = $1 AND perkid = 2", ctx.author.id)
        dug_ores = player_shovel.dig(has_perk_2=has_perk_2)

        if len(dug_ores) == 0:
            mins /= 2
            return await ctx.send(f"{random.choice(self.none_found_messages)} +{len(dug_ores)} ores, ready again {discord.utils.format_dt(discord.utils.utcnow() + timedelta(minutes=mins), 'R')}")
        ...
        await self.bot.pool.execute("""
            UPDATE e_treasure SET
                ...
                lastmins = $2
            WHERE id = $3
        """, discord.utils.utcnow(), mins, player.id)

        neat_ores = ""
        for o in dug_ores:
            await self.bot.pool.execute("INSERT INTO e_treasure_inventory(id, oreid, minedwith, dugwhen) VALUES ($1, $2, $3, $4)", ctx.author.id, o.id, player_shovel.id, discord.utils.utcnow())
            neat_ores = neat_ores + f"{len([oc for oc in dug_ores if oc.id == o.id])}x {o.name}, "
        mins_ts = discord.utils.format_dt(discord.utils.utcnow() + timedelta(minutes=mins), "R")
        await ctx.send(f"{random.choice(self.found_messages)} +{len(dug_ores)} ores, ready again {mins_ts}\n-# {neat_ores.removesuffix(', ')}{perk_message}")

    @commands.command(aliases=['ore'])
    async def ores(self, ctx):
        """Shows a list of obtainable ores"""
        player = await Player.get(ctx.author.id, self.bot)
        embed = discord.Embed(title="\U0001faa8 Treasure Ores", color=player.profile_color)
        ore_list = 'id  |   name    |  cost  | rarity\n----+-----------+--------+-------\n' + '\n'.join([f"{ore.id}{'  ' if len(str(ore.id)) == 1 else ' '} | {ore.name}{(' ')*(11-(len(ore.name)+1))}| ~${ore.sell_cost}{(' ')*(8-(len(str(ore.sell_cost))+3))}| {ore.rarity}" for ore in REWARDS])
    ...
    async def inventory(self, ctx):
        ...
        player = await Player.get(ctx.author.id, self.bot)
        embed = discord.Embed(title="\U0001f392 Treasure Inventory", color=player.profile_color)
        ores = [tuple(o) for o in (await self.bot.pool.fetch("SELECT * FROM e_treasure_inventory WHERE id = $1", ctx.author.id))]
        ore_counts = {}
        if len(ores) == 0:
            embed.description = "*Its looking empty in here...*"
        for o in ores:
            try: ore_counts[o[1]] += 1
            except KeyError: ore_counts[o[1]] = 1
        total_sell_estimate = 0
        for key, value in sorted(ore_counts.items()):
            ore = Ore.get(key)
            embed.add_field(name=f"{value}x {ore.name}", value=f"~${ore.sell_cost * value}")
        ...
        if total_sell_estimate != 0:
            ...
            embed.set_footer(text=f"Do {ctx.clean_prefix}dig to start digging for treasure!")
    ...
    async def sell_(self, ctx):
        ...
        player = await Player.get(ctx.author.id, self.bot)
        has_sell_perk = False
        inventory = await self.bot.pool.fetch("SELECT * FROM e_treasure_inventory WHERE id = $1", ctx.author.id)
        if len(inventory) == 0:
            return await ctx.send("\U0001f392 ... Your inventory is empty")
        selltotal = 0
        for entry in inventory:
            selltotal += Ore.get(entry.get('oreid')).sell_cost * Shovel.get(entry.get('minedwith')).multiplier
        activePerks = await self.bot.pool.fetch("SELECT perkid FROM e_treasure_perks WHERE id = $1", ctx.author.id)
        if 3 in [p.get('perkid') for p in activePerks]:
            has_sell_perk = True
            selltotal *= 1.30 # 30% sell perk
        conf = Confirm()
        conf_msg = await ctx.send(f"Sell the treasures in your inventory for ${int(selltotal)}? {'*+30% from Rock Polisher perk*' if has_sell_perk else ''}", view=conf)
        await conf.wait()
        if conf.value:
            await conf_msg.delete()
            await self.bot.pool.execute("DELETE FROM e_treasure_inventory WHERE id = $1", ctx.author.id)
            await player.update_balance(amount=selltotal, ctx=ctx)
            if has_sell_perk:
                await self.bot.pool.execute("DELETE FROM e_treasure_perks WHERE id = $1 AND perkid = 3", ctx.author.id)
