...
class lottery(commands.Cog, command_attrs=dict(name='Lottery')):
    ...
    async def reset_lottery_time(self):
        ...
        await self.bot.pool.execute("UPDATE e_lottery_main SET drawingwhen = $1", newdrawingtime)
    ...
    async def draw(self, force: bool = False):
        ...
        if data is None:
            ...
            await self.bot.pool.execute("INSERT INTO e_lottery_main VALUES ($1, 1)", date)
            return
        piss = data[0]
        ...
        if diff < 0 or force:
            ...
            winningplayer = await self.bot.pool.fetchrow("SELECT * FROM e_lottery_users ORDER BY RANDOM()")
            ...
            try:
                ...
                await self.bot.pool.execute("UPDATE e_users SET bal = (bal + $1) WHERE id = $2", winningamount,winningplayer[0])
                await self.bot.pool.execute("UPDATE e_users SET lotterieswon = (lotterieswon + 1) WHERE id = $1", winningplayer[0])
                await self.bot.pool.execute("DELETE FROM e_lottery_users")
                await self.bot.pool.execute("UPDATE e_lottery_main SET drawingnum = (drawingnum + 1)")
                await self.bot.stats.add('totalLotteries')
                log.info(f"Lottery draw, {user.name} won ${winningamount}. Reset successfully")
    ...
    async def buy(self, ctx):
        ...
        if player.balance < 100:
            ...
            return
        lcheck = await self.bot.pool.fetchrow("SELECT * FROM e_lottery_users WHERE userid = $1", ctx.author.id)
        ...
        await self.bot.pool.execute("INSERT INTO e_lottery_users VALUES ($1, $2, $3)", ctx.author.id, ctx.author.name, discord.utils.utcnow())
